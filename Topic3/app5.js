...
function renderDevices() {
  $.get(`${API_URL}/securitysystem`)
    .then(response => {
      $('#deviceese tbody').empty();
      response.forEach(device => {
        $('#deviceese tbody').append(`
          <tr>
            <td>${device.user}</td>
            <td>${device.name}</td>
...
$('#add-devicess').on('click', () => {
  ...
  $.post(`${API_URL}/securitysystem`, body)
    .then(response => {
      console.log('eqqqqqqqq');
      window.location.replace('http://localhost:3001/amwe') ;
    })
    .catch(error => {
...
async function deleteDevice(idToDelete) {
  try {
    ...
    const response = await fetch(`${API_URL}/securitysystem/${idToDelete}`, {
      method: 'DELETE'
    });
    if (!response.ok) {
...
// Event listener for delete button
$('#deviceese tbody').on('click', '.delete-btn', function () {
